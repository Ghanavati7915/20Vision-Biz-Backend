...
export class UploadService {
   ...
   async upload(folderName, file: Express.Multer.File) {
      try {
         ...
         this.validateFileExtension(validation, `.${this.getFileExtension(file.originalname)}`);
   ...
   validateFileExtension(regex: RegExp, fileExtension: string) {
      if (!regex.test(fileExtension)) throw new BadRequestException("فایل را با فرمت صحیح وارد کنید");
   }
   //#endregion
}
