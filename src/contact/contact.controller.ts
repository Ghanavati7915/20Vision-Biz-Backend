...
export class ContactController {
  ...
  createUser(@Request() req: any, @Body() payload: ContactUserDto) {
    return this.contactService.createUser(
      req.user.id,
      req.user.userID,
      payload,
    );
  ...
  createCompany(@Request() req: any, @Body() payload: ContactCompanyDto) {
    return this.contactService.createCompany(
      req.user.id,
      req.user.userID,
      payload,
    );
  ...
  @ApiResponse({ status: 400, description: 'Bad request' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  toggleCompanyCustomerState(
    @Request() req: any,
    @Body() payload: ToggleStateDto,
    @Param('id') id: string,
  ) {
    return this.contactService.toggleCompanyCustomerState(
      req.user.id,
      req.user.userID,
      payload,
      +id,
  ...
  @ApiResponse({ status: 400, description: 'Bad request' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  toggleUserCustomerState(
    @Request() req: any,
    @Body() payload: ToggleStateDto,
    @Param('id') id: string,
  ) {
    return this.contactService.toggleUserCustomerState(
      req.user.id,
      req.user.userID,
      payload,
      +id,
  ...
  getAll(@Request() req: any, @Body() filter: FilterContactDto) {
    return this.contactService.getAll(req.user.tenantId, filter);
  }
  //#endregion
